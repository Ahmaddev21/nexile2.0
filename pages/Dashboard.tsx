                     ...
                     {insights.length === 0 ? (
                        ...
                        <div className="space-y-3">
                           {insights.map((insight, idx) => (
                              <div key={idx} className={`p-4 rounded-xl border ${insight.type === 'warning' ? 'bg-amber-50 border-amber-100 text-amber-800' :
            ...
            <div className="bg-white dark:bg-dark-card rounded-3xl border border-slate-100 dark:border-dark-border shadow-card overflow-hidden transition-all hover:shadow-card-hover">
               ...
               <div className="p-4 space-y-2">
                  ...
                  {insights.map((insight: any, idx: number) => (
                     <div key={idx} className={`p-4 rounded-2xl border ${insight.type === 'warning'
                     ...
                     <div className="space-y-3 flex-1 overflow-y-auto pr-2 custom-scrollbar relative z-10">
                        {insights.map((insight: any, i: number) => (
                           <div key={i} className="p-3 bg-white/10 rounded-xl backdrop-blur-sm border border-white/5">
                              <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded mb-1 inline-block ${insight.type === 'warning' ? 'bg-rose-500/20 text-rose-300' : 'bg-emerald-500/20 text-emerald-300'
                                 }`}>
                                 {insight.metric}
                              </span>
                     ...
                     return (
                        <div key={branch.id} onClick={() => setSelectedBranch(branch)} className="bg-white dark:bg-dark-card p-6 rounded-3xl border border-slate-100 dark:border-dark-border shadow-card flex items-center justify-between hover:shadow-card-hover transition-shadow group relative overflow-hidden cursor-pointer">
                           <div className="flex-1 mr-4">
                              ...
                              <div className="mt-4 flex items-center gap-2 text-sm" onClick={e => { e.stopPropagation(); }}>
                     ...
                     <div className="bg-brand-50 dark:bg-brand-900/20 border border-brand-100 dark:border-brand-800 rounded-2xl p-6 text-center animate-fadeIn relative overflow-hidden">
                        ...
                        <div className="flex gap-2">
                           <button
                              ...
                              className={`flex-1 flex items-center justify-center py-2 rounded-lg text-xs font-bold shadow-sm transition-colors ${isCopied
